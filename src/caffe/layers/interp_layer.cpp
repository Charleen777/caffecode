...
void InterpLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  int num_specs = 0;
  num_specs += this->layer_param_.interp_param().has_zoom_factor();
  num_specs += this->layer_param_.interp_param().has_shrink_factor();
  num_specs += this->layer_param_.interp_param().has_height() && this->layer_param_.interp_param().has_width();
  CHECK_EQ(num_specs, 1) << "Output dimension specified either by "
			 << "zoom factor or shrink factor or explicitly";
  pad_beg_ = this->layer_param_.interp_param().pad_beg();
  pad_end_ = this->layer_param_.interp_param().pad_end();
...
void InterpLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  //InterpParameter interp_param = this->layer_param_.interp_param();
  if (this->layer_param_.interp_param().has_zoom_factor()) {
    const int zoom_factor = this->layer_param_.interp_param().zoom_factor();
  ...
  else if (this->layer_param_.interp_param().has_shrink_factor()) {
    const int shrink_factor = this->layer_param_.interp_param().shrink_factor();
  ...
  else if (this->layer_param_.interp_param().has_height() && this->layer_param_.interp_param().has_width()) {
    height_out_  = this->layer_param_.interp_param().height();
    width_out_  = this->layer_param_.interp_param().width();
  }
  else {
