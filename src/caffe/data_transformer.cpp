...
void DataTransformer<Dtype>::TransformImgAndSeg(const std::vector<cv::Mat>& cv_img_seg,
  ...
  for (int h = 0; h < data_height; ++h) {
	...
	top_index = h * data_width + w;
      ...
      //no flip for left and right items
      if(do_mirror) {//only applicable for human parsing task
         if(transformed_label[top_index] == 9) { //leftshoe->rightshoe
             transformed_label[top_index] = 10;
         } else if(transformed_label[top_index] == 10) { //rightshoe->leftshoe
              transformed_label[top_index] = 9;
         } else if(transformed_label[top_index] == 12) { //leftleg -> rightleg
             transformed_label[top_index] = 13;
         } else if(transformed_label[top_index] == 13) { //rightleg->leftleg
             transformed_label[top_index] = 12;
         } else if(transformed_label[top_index] == 14) { //leftarm->rightarm
             transformed_label[top_index] = 15;
         } else if(transformed_label[top_index] == 15) { //rightarm->leftarm
             transformed_label[top_index] = 14;
         }
